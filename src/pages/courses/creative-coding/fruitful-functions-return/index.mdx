...
<CodeExample live animated canvasHeight={200} code={`function setup() {
...
<CodeExample live animated canvasHeight={200} preamble={`function setup() { createCanvas(windowWidth, windowHeight); background(100); }`} code={`function draw() {
	squareAndCircle();
}
