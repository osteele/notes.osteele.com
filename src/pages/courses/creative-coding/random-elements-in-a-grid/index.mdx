...
<CodeExample live canvasHeight={400} code={`let rows = 5;
let columns = 6;
...
}`} lang="jsx" image="/images/random-elements-in-a-grid/random-elements-in-a-grid-12.png" alt="" />
...
<CodeExample live canvasHeight={400} preamble={`let rows = 5;\nlet columns = 6;\nfunction setup() { createCanvas(windowWidth, windowHeight); }`} code={`function draw() {
	background(200);
	for (let row = 0; row < rows; row++) {
		for (let column = 0; column < columns; column++) {
			let x = 30 + 50 * column;
...
}`} lang="jsx" image="/images/random-elements-in-a-grid/random-elements-in-a-grid-02.png" alt="" />
...
}`} lang="jsx" image="/images/random-elements-in-a-grid/random-elements-in-a-grid-04.png" alt="" />
...
}`} lang="jsx" image="/images/random-elements-in-a-grid/random-elements-in-a-grid-05.png" alt="" />
...
}`} lang="jsx" image="/images/random-elements-in-a-grid/random-elements-in-a-grid-06.png" alt="" />
...
}`} lang="jsx" image="/images/random-elements-in-a-grid/random-elements-in-a-grid-07.png" alt="" />

Note: For the following code samples, the values of `rows` and `columns` have been changed to 20.

<CodeExample live canvasHeight={400} preamble={`let rows = 20;\nlet columns = 20;\nfunction setup() { createCanvas(windowWidth, windowHeight); }`} code={`function draw() {
	background(200);
	for (let row = 0; row < rows; row++) {
		for (let column = 0; column < columns; column++) {
			let x = 30 + 50 * column;
...
}`} lang="jsx" image="/images/random-elements-in-a-grid/random-elements-in-a-grid-08.png" alt="" />
...
}`} lang="jsx" image="/images/random-elements-in-a-grid/random-elements-in-a-grid-09.png" alt="" />
...
}`} lang="jsx" image="/images/random-elements-in-a-grid/random-elements-in-a-grid-10.png" alt="" />
...
<CodeExample live animated canvasHeight={400} preamble={`let rows = 5;\nlet columns = 6;\nfunction setup() { createCanvas(windowWidth, windowHeight); }`} code={`function draw() {
	background(200);
	for (let row = 0; row < rows; row++) {
		for (let column = 0; column < columns; column++) {
			let x = 30 + 50 * column;
...
}`} lang="jsx" image="/images/random-elements-in-a-grid/random-elements-in-a-grid-11.png" alt="" />
...
<CodeExample live canvasHeight={400} preamble={`let rows = 20;\nlet columns = 20;`} code={`function setup() {
	createCanvas(windowWidth, windowHeight);
	rectMode(CENTER);
}
...
<CodeExample live animated canvasHeight={400} preamble={`let rows = 20;\nlet columns = 20;\nfunction setup() { createCanvas(windowWidth, windowHeight); rectMode(CENTER); }`} code={`function draw() {
	background(200);
	for (let row = 0; row < rows; row++) {
		for (let column = 0; column < columns; column++) {
			let x = 30 + 50 * column;
...
Fourth approach: generate the random values once in `setup()`, and store them in an array. Then `draw()` reads from the array instead of calling `random()`. Since the array doesn't change between frames, the pattern stays the same.

<CodeExample live canvasHeight={400} preamble={`let rows = 5;\nlet columns = 6;`} code={`let isCircle = [];
...
function setup() {
	...
	for (let row = 0; row < rows; row++) {
		isCircle[row] = [];
		for (let column = 0; column < columns; column++) {
			isCircle[row][column] = random() < 1/4;
		}
	}
}
...
Like the `noise()` approach, this lets us keep animating without flicker, because `draw()` uses the stored values rather than generating new random ones each frame.

<CodeExample live animated canvasHeight={400} preamble={`let rows = 20;\nlet columns = 20;`} code={`let isCircle = [];
