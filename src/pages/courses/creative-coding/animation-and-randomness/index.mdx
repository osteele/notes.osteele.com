...
- ***Time***. In p5.js, the `frameCount` variable and the `millis()` function are the sources of time. We explored that in [Time-Based Animation](#time-based-animation).
- A ***global variable***. You can define your own variable that has global scope (is defined outside of `setup()` and `draw()`), and use it to remember changes from one frame to the next. There was an example of this in [Update-Based Animation](#update-based-or-incremental-animation).
- ***Randomness***. The `random()` function returns a different number each time you use it. This will cause a frame that calls `random()` to draw differently each frame. There were an examples of this in [Random](#random) and [Noise](#noise).
