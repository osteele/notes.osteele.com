...
These two functions are equivalent. They each initialize `y` to 50, increment it by 50 each time, draw circles while `y` is less or equal to 200.
...
- It is more common to count from 0 using `<` than to count from 1 and use `<=` or `==`. When we cover arrays, we will see why.
